...
def calc_all_year(flow_matrix):
    ...
    for index, _ in enumerate(flow_matrix[0]):
        average_annual_flows.append(
            round(np.nanmean(flow_matrix[:, index]), 2))
        standard_deviations.append(round(np.nanstd(flow_matrix[:, index]), 2))
        coefficient_variations.append(round(
            standard_deviations[-1] / average_annual_flows[-1], 2))
